        ...
        <q-table :rows="data" :columns="columns" hide-bottom>
          <template v-slot:body-cell-Name="props">
            <q-td :props="props">
              <q-item style="max-width: 420px">
                ...
                <q-item-section v-if="props.row.userId">
          ...
          <template v-slot:body-cell-Action="props">
            <q-td :props="props">
              ...
              <q-btn icon="delete"  @click="onDelete(props.row._id)" size="sm" class="q-ml-sm" flat dense />
            </q-td>
          </template>
        </q-table>
      </q-card-section>
...
export default {
  ...
  methods: {
    ...
    async onDelete(id) {
      try {
        await postCategoriesService.deleteCat(id).then((response) => {
          this.$q.notify({
            type: "positive",
            timeout: 1000,
            position: "center",
    ...
    async addTask() {
      try {
        await postCategoriesService.postCategories(this.post).then((response) => {
          this.$q.notify({
            type: "positive",
            timeout: 1000,
            position: "center",
